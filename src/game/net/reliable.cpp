...
int ReliableCommClass::Connect(void *addr, int len, int type)
{
    ...
    if (type == 0) // Sole 1.0 has this as UnreliableCommClass::Resolve_Address
    {
        ...
        if (Host.Addr != -1) {
            ...
            if (connect(Socket, (sockaddr *)&name, sizeof(name)) == SOCKET_ERROR && LastSocketError != SOCKEWOULDBLOCK) {
...
int ReliableCommClass::Send()
{
    ...
    if (SendEntry) {
        SendLen = SendEntry->m_BufLen;
        buff = (uint16_t *)SendEntry->m_Buffer;
        *buff = htons(SendEntry->m_BufLen);
...
long __stdcall ReliableCommClass::Window_Proc(HWND hwnd, unsigned int message, unsigned int wparam, long lparam)
{
    ...
    switch (message) {
        ...
        case SSWM_HOSTBYADDR_MSG:
            captainslog_info("ReliableComms resolved a host by address.");
        ...
        case SSWM_HOSTBYNAME_MSG:
            captainslog_info("ReliableComms resolved a host by name.");
            if (WSAGETSELECTERROR(lparam) != 0) {
                if (thisptr->Protocol != nullptr) {
        ...
        case SSWM_RELIABLE_COMM_MSG:
            captainslog_info("ReliableComms handling a packet.");
            switch (WSAGETSELECTEVENT(lparam)) {
                case FD_READ:
                    if (WSAGETSELECTERROR(lparam) != 0 && WSAGETSELECTERROR(lparam) != SOCKEWOULDBLOCK) {
                        if (thisptr->Protocol != nullptr) {
                    ...
                    while (true) {
                        ...
                        if (thisptr->ReceiveLen < 2) {
                            ...
                            if (strncmp(thisptr->ReceiveBuf, "GE", sizeof(unsigned short)) == 0) {
                                ...
                                *((unsigned short *)thisptr->ReceiveBuf) = std::min((int)packet_size, thisptr->MaxPacketSize);
                                get_request = false;
                            }
                        }
                ...
                case FD_WRITE: {
                    ...
                    while (thisptr->SendLen > 0) {
                        int sent = send(thisptr->Socket, (const char *)&thisptr->SendEntry->m_Buffer[packet_size - thisptr->SendLen], thisptr->SendLen, 0);
